...
function print_help_menu() {
    ...
    ###########################################################################
                  This is the help menu for SequelQC version 0.9.9
    ###########################################################################
...
while getopts ":u:c:n:g:p:vkh" opt; do
    case ${opt} in 
      ...
      g )
        ...
        ;;
      p )
        if [ $OPTARG == "b" -o $OPTARG == "i" -o $OPTARG == "a" ]; then
...
FILES_FOR_R=""
LENGTHS_FOR_R=""
L=12         #the number of inputs to pass to R (per scraps/subreads pair)
I=1
